 ...
 * instruction. Wan's 3.5 and Qwen's 2.5-4 are a different quantity that happens
 * to share the scale: a real CFG over a real negative branch, not FLUX's
 * distilled guidance embedding. Mirrors the backend's `_default_guidance`.
 * Resolves through `resolveFlux` so the guidance follows the model that will
 * actually run, not whichever one happens to sort first. */
 ...
 * wants more to hit its ceiling, and the Qwen checkpoints, which each publish
 * their own. Mirrors the backend's `_default_steps`; resolves through
