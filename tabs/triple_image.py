...
import io
from utils import combine_three_images_side_by_side, encode_image_to_base64, stream_ollama_chat
...
class TripleImageTab:
    def __init__(self, ollama_url, model_name):
        ...
        if "system_image_triple_b64" not in st.session_state:
            st.session_state.system_image_triple_b64 = None
        if "system_image_triple_name" not in st.session_state:
            st.session_state.system_image_triple_name = None
    ...
    def _render_chat_section(self):
        ...
        with st.expander("💬 Custom System Prompt (optional)", expanded=False):
            ...
            system_image = st.file_uploader(
                "System prompt image",
                type=["jpg", "jpeg", "png", "bmp", "gif", "webp"],
                key="system_image_triple",
                help="Upload an image to attach to the system prompt. It will be sent with every message.",
                label_visibility="collapsed"
            )
            
            if system_image:
                st.session_state.system_image_triple_b64 = encode_image_to_base64(system_image)
                st.session_state.system_image_triple_name = system_image.name
                st.image(system_image, caption=f"System Image: {system_image.name}", width=200)
            elif st.session_state.system_image_triple_b64:
                st.image(f"data:image/png;base64,{st.session_state.system_image_triple_b64}", 
                        caption=f"System Image: {st.session_state.system_image_triple_name}", width=200)
                if st.button("🗑️ Remove System Image", key="remove_system_image_triple"):
                    st.session_state.system_image_triple_b64 = None
                    st.session_state.system_image_triple_name = None
                    st.rerun()
    ...
    def _build_messages(self, prompt):
        ...
        if st.session_state.get('system_prompt_triple', '').strip():
            ...
            if st.session_state.system_image_triple_b64:
                system_msg["images"] = [st.session_state.system_image_triple_b64]
            messages.append(system_msg)
        
        # Add conversation history
        history = st.session_state.messages_triple
        for msg in history:
            # Get the original index for image attachment logic
            original_i = st.session_state.messages_triple.index(msg)
            if msg["role"] == "user":
                user_msg = {
                    "role": msg["role"],
                    "content": msg["content"]
