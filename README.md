...
| **Qwen-Image 2512 + Edit 2511** — fp8 | ~51 GB | 28 GB | create + edit + combine + control | The best there is at legible text inside an image, English and Chinese. Ungated. |
| **Qwen-Image + Edit** — fp8 | ~51 GB | 28 GB | create + edit + combine + control | The August originals; superseded by the pair above. ~41 GB if 2512 is installed, which it shares an encoder and VAE with. |
| **Wan 2.2 I2V A14B** — fp16 | ~69 GB | 80 GB | animate | One image → a 5-second 720p video. Ungated. |

Qwen splits create and edit across two separately fine-tuned checkpoints of one 20B architecture —
the shape FLUX.1 had, where dev created and Kontext edited — so a Qwen bundle ships **both
transformers**, paired by generation, and one install covers every image mode. They are ~51 GB
because that is two 20 GB transformers plus a shared 9.4 GB text encoder and VAE, not two 30 GB
models. **VRAM does not go up**: only one transformer is resident at a time, and alternating between
Create and Edit costs a reload rather than a second card.

There is no "Qwen-Image Edit 2512" — 2512 was a text-to-image refresh, and 2511 is the current edit
model, which is why the December pair is 2512 + 2511.

Within a bundle the two halves are not interchangeable, and the model picker lists them separately.
The create half cannot edit (a plain text encode has nowhere to put a source image). The edit half
does everything: edit, combine, control, and — since its encode node takes images optionally — plain
text-to-image too. Its ceiling is references: **3 images at once on Edit 2511, 1 on the original**,
and a combine that exceeds it is refused rather than quietly dropping the extras.
